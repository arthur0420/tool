...
public class SerialPortTest1 implements Runnable, SerialPortEventListener {
	 static String comName = null; // 配置文件
    ...
    public void init() {
        ...
        while (portList.hasMoreElements()) {
            ...
            if (portId.getPortType() == CommPortIdentifier.PORT_SERIAL) {
            	System.out.println("sss:"+portId.getName());
                // 比较串口名称是否是"COM3"
    ...
    public void readComm() {
        ...
        try {
            ...
            while ((len = inputStream.read(readBuffer)) != -1) {
                System.out.println("receive：" + new String(readBuffer, 0, len).trim() +","+ new Date());
                break;
            }
        } catch (Exception e) {
            e.printStackTrace();
    ...
    //向串口发送数据
    public void sendMsg(){
        String information = "test ab cd 123 from "+comName;
        try {
            //实例化输出流
